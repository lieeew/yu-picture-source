<template>
  <div id="addPictureTagsBatchPage">
    <h2 style="margin-bottom: 16px">批量智能标签</h2>
    ...
    <a-card style="margin-top: 16px">
      <a-space direction="vertical" style="width: 100%">
        <a-button
          ...
          @click="handleSubmit"
        >
          执行批量智能标签 (已选 {{ selectedRowKeys.length }} 张)
        </a-button>
...
import { generatePictureTagsByBatch, listPictureTasks } from '@/api/pictureController.ts'
...
const handleSubmit = async () => {
  ...
  try {
    const res = await generatePictureTagsByBatch({
